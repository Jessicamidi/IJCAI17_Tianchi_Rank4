...
PRECIP = pd.read_csv('../external/PRECIP.csv')
...
#%%
WEATHER = pd.read_csv('../external/WEATHER_extract.csv')
...
#MOVIE_A =  [(lambda x:('NAA'+ str(x).zfill(2))) (x)  for x in range(TST_N)]
#MOVIE_B =  [(lambda x:('NAB'+ str(x).zfill(2))) (x)  for x in range(TST_N)]
#MOVIE_C =  [(lambda x:('NAC'+ str(x).zfill(2))) (x)  for x in range(TST_N)]
#MOVIE_D =  [(lambda x:('NAD'+ str(x).zfill(2))) (x)  for x in range(TST_N)]
#MOVIE_E =  [(lambda x:('NAE'+ str(x).zfill(2))) (x)  for x in range(TST_N)]
#
#MOVIE_columns = MOVIE_A + MOVIE_B + MOVIE_C + MOVIE_D + MOVIE_E
#
#MOV_IND = pd.read_csv('../external/MOV_INDEX.csv')
#MOV_IND = MOV_IND[['DATE','Num','RAT_d3','RAT_w1','RAT_w3','RAT_w5']]
#MOV_IND.set_index(keys = ['DATE'],drop = True,inplace = True)
#MOV_TAB = MOV_IND.transpose()
#
#
#MOV_TST = pd.DataFrame()
#for TRN_STA,TRN_END,TST_STA,TST_END in date_all:
#    tt = MOV_TAB.loc[:,TST_STA:TST_END]
#    tt3 = tt.stack().to_frame().T    
#    tt3.columns = MOVIE_columns
#    tt3.loc[:,'TST_STA'] = TST_STA
#    tt3.loc[:,'TST_END'] = TST_END     
#    MOV_TST = pd.concat([MOV_TST,tt3])


#TRAIN_OK = pd.merge(TRAIN_OK , MOV_TST,on=['TST_STA','TST_END'],how = 'left')
#TEST = pd.merge(TEST , MOV_TST,on=['TST_STA','TST_END'],how = 'left')
...
X.to_csv('../XY/0313_X.csv', index = False)
Y.to_csv('../XY/0313_Y.csv', index = False)
X_test.to_csv('../XY/0313_Xtest.csv', index = False)
